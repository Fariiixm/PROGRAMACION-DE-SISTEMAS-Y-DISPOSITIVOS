...
void lcd_draw_hrow(uint16 xleft, uint16 xright, uint16 y, uint8 color,
		uint16 width) {
...
void lcd_draw_vrow(uint16 yup, uint16 ydown, uint16 x, uint8 color,
		uint16 width) {
	uint8 width_aux = width;
	while (yup <= ydown) {
		width_aux--;
...
void lcd_draw_box(uint16 xleft, uint16 yup, uint16 xright, uint16 ydown,
	...
	lcd_draw_hrow(xleft, xright, ydown - width + 1, color, width);
	lcd_draw_vrow(yup, ydown, xright - width + 1, color, width);
